...
import sys  # Needed for PyInstaller compatibility
...
class VoiceToTextApp:
    def __init__(self, root):
        ...
        #For handling paths for PyInstaller
        #A fix for LanguageTool .jar path when running as a bundled app
        if getattr(sys, 'frozen', False):
            self.jar_path = os.path.join(sys._MEIPASS, "languagetool-commandline.jar")
        else:
            self.jar_path = os.path.join(os.path.dirname(__file__), "languagetool-commandline.jar")

        os.environ["TOOL_PATH"] = self.jar_path  # Required by language_tool_python
        ...
        self.recognizer = KaldiRecognizer(self.model, 16000)  # Sample rate must match model
